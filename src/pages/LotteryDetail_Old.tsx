...
type Participant = (typeof mockParticipants)[number];
...
export default function LotteryDetail() {
  ...
  const resolvedLottery =
    ...
    (mockLotteries as LotteryData[]).find(
  ...
  const [participants, setParticipants] = useState<Participant[]>(
    mockParticipants.filter((p) => p.lotteryId === lotteryKey),
  );
  ...
  const [lottery, setLottery] = useState<LotteryData | undefined>(resolvedLottery);
  ...
  const endLottery = () => {
    if (
      ...
      setLottery((current) => (current ? { ...current, status: "drawing" } : current));
    }
  };

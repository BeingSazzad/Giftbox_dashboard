...
export default function Finance() {
  ...
  const totalRevenue = mockLotteries.reduce((acc, curr) => acc + curr.revenue, 0)
  const ticketsSold = mockLotteries.reduce((acc, curr) => acc + curr.participants, 0)
    ...
    <div>
      <div className="section-header mb-6">
        <div>
          ...
          <div className="section-sub">Manage revenue, currency, and mobile money gateways</div>
        </div>
      ...
      <div className="metric-grid" style={{ gridTemplateColumns: 'repeat(3, 1fr)', marginBottom: 30 }}>
        <div className="metric-card gold" style={{ flexDirection: 'row', alignItems: 'center', gap: 16, padding: '20px 16px' }}>
          <div className="metric-icon gold" style={{ width: 48, height: 48, flexShrink: 0 }}>
            <Wallet size={22} strokeWidth={2.5} />
          </div>
          <div style={{ flex: 1 }}>
            <div className="metric-label" style={{ fontSize: '11px', textTransform: 'uppercase', color: 'var(--text-muted)', marginBottom: '2px' }}>Total Revenue</div>
            <div className="metric-value" style={{ fontSize: '24px', fontFamily: 'Space Grotesk, sans-serif' }}>
              {totalRevenue.toLocaleString()} <span style={{ fontSize: 13, fontWeight: 500, opacity: 0.8 }}>{currency}</span>
            </div>
          </div>
        </div>
        ...
        <div className="metric-card green" style={{ flexDirection: 'row', alignItems: 'center', gap: 16, padding: '20px 16px' }}>
          <div className="metric-icon green" style={{ width: 48, height: 48, flexShrink: 0 }}>
            <TrendingUp size={22} strokeWidth={2.5} />
          </div>
          <div style={{ flex: 1 }}>
            <div className="metric-label" style={{ fontSize: '11px', textTransform: 'uppercase', color: 'var(--text-muted)', marginBottom: '2px' }}>Avg Ticket Price</div>
            <div className="metric-value" style={{ fontSize: '24px', fontFamily: 'Space Grotesk, sans-serif' }}>
              2,500 <span style={{ fontSize: 13, fontWeight: 500, opacity: 0.8 }}>{currency}</span>
            </div>
          </div>
        </div>
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 24, alignItems: 'start' }}>
        ...
        <div className="settings-block" style={{ margin: 0 }}>
          <div className="settings-block-header">
            ...
            <span style={{ marginLeft: 'auto', fontSize: 11, color: 'var(--text-muted)', fontWeight: 400 }}>Displayed to users</span>
          </div>
          <div style={{ padding: '16px 20px 20px', fontSize: 13, color: 'var(--text-muted)' }}>
            Configure the mobile money numbers that users will send their payments to. Ensure these numbers are active and monitored.
          </div>
        ...
        <div className="settings-block" style={{ margin: 0 }}>
          ...
          <div className="settings-row" style={{ borderBottom: 'none' }}>
            <div>
              ...
              <div className="settings-row-desc">System-wide currency used for tickets and revenue calculation</div>
            </div>
            <select
